...
push_name = "amuvarma/5k-luna-tune-instruct-contentonly"

ds_name = "amuvarma/luna-4days-combined-clean-wcaps"
ds = load_dataset(ds_name)

ds = ds["train"]
ds = ds.filter(lambda x: x["emotion"] in ["happy", "normal", "curious", "angry"])
print(ds)
...
def tokenize_and_add_to_dataset(dataset):
    def tokenize_transcript(example):
        ...
        # possible_emotion = ["neutral", "happy", "sad", "angry", "fearful", "disgusted", "surprised", "calm", "disappointed", "excited", "frustrated", "proud", "relieved", "thankful", "hopeful", "apprehensive", "anxious", "confused", "curious", "embarrassed", "envious", "guilty", "jealous", "lonely", "nostalgic", "sentimental", "sympathetic", "ashamed", "bored", "distracted", "determined", "devastated", "grateful", "impressed", "indifferent", "interested", "joyful", "peaceful", "pessimistic", "satisfied", "shocked", "skeptical", "stressed", "terrified", "uncomfortable", "worried"]
        # user_emotion = random.choice(possible_emotion)
        # user_emotion = example["emotion"]


        user_text = example["text"]

        instruction_phrase_string = user_text

        tokenized = tokeniser(example["text"])
        tokenized_text = tokenized['input_ids'] + [end_of_text]

        tokenised_instruction = tokeniser(instruction_phrase_string)
        tokenised_instruction = tokenised_instruction['input_ids'] + [end_of_text]
        ...
        example['tokenised_instruction'] = tokenised_instruction
...
def create_input_ids(example):
    input_ids = [start_of_human]+ example["tokenised_instruction"]  + [end_of_human, start_of_ai] + example['tokenised_text'] + [start_of_speech]
