...
push_name = "amuvarma/paired-ds-luna-3-train-tagged-processed-1"

ds_name = "amuvarma/paired-ds-luna-3-train"
...
fac_order_2 = ['facodec_1_2', "facodec_0_2", "facodec_2_2",  "facodec_3_2",  "facodec_4_2",  "facodec_5_2"]
...
def process_dataset(dataset):
    ...
    def add_values_2(example):
        for i, col in enumerate(fac_order_2):
            example[col] = [x + audio_tokens_start + i * 1024 for x in example[col]]
        return example
    
    # Apply the transformations
    dataset = dataset.map(lambda x: {col: x[col] for col in fac_order})
    dataset = dataset.map(lambda x: {col: x[col] for col in fac_order_2})
    dataset = dataset.map(add_values)
    dataset = dataset.map(add_values_2)
...
def tokenize_and_add_to_dataset(dataset):
    def tokenize_transcript(example):
        ...
        tokenized_2 = tokeniser(example["transcript_2"])
        

        tokenized_text = tokenized['input_ids'] + [end_of_text]
        tokenized_text_2 = tokenized_2['input_ids'] + [end_of_text]
        
        # Add the new tokenised_text to the examsple
        example['tokenised_text'] = tokenized_text
        example['tokenised_text_2'] = tokenized_text_2
...
def create_input_ids(example):
    ...
    input_ids_2 = [start_of_human] + example['tokenised_text_2'] + [end_of_human, start_of_ai, start_of_speech]

    # Interleave the facodec lists
    max_len = max(len(example[facodec]) for facodec in fac_order_2)

    for i in range(max_len):
        for facodec in fac_order_2:
            if i < len(example[facodec]):
                input_ids_2.append(example[facodec][i])

    input_ids_2 += [end_of_speech, end_of_ai]
